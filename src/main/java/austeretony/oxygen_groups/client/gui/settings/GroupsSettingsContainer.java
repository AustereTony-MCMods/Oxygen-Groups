...
public class GroupsSettingsContainer implements ElementsContainer {
    ...
    private OxygenDropDownList alignmentGroupMenu, statusBarText;
    ...
    public void addGUI(GUIElementsFramework framework) {
        ...
        this.alignmentGroupMenu.addElement(new OxygenDropDownListEntry<Integer>(- 1, ClientReference.localize("oxygen_core.alignment.left")));
        this.alignmentGroupMenu.addElement(new OxygenDropDownListEntry<Integer>(0, ClientReference.localize("oxygen_core.alignment.center")));
        this.alignmentGroupMenu.addElement(new OxygenDropDownListEntry<Integer>(1, ClientReference.localize("oxygen_core.alignment.right")));

        this.alignmentGroupMenu.<OxygenDropDownListEntry<Integer>>setClickListener((element)->{
            EnumGroupsGUISetting.GROUP_MENU_ALIGNMENT.get().setValue(String.valueOf(element.index));
            OxygenManagerClient.instance().getClientSettingManager().changed();
        });
        ...
        for (EnumStatusBarText enumText : EnumStatusBarText.values()) 
            this.statusBarText.addElement(new OxygenDropDownListEntry<Integer>(enumText.ordinal(), enumText.getLocalizedDescription()));

        this.statusBarText.<OxygenDropDownListEntry<Integer>>setClickListener((element)->{
            EnumGroupsGUISetting.OVERLAY_STATUS_BAR_TEXT.get().setValue(String.valueOf(element.index));
            OxygenManagerClient.instance().getClientSettingManager().changed();
        });
