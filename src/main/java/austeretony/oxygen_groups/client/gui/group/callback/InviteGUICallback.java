...
public class InviteGUICallback extends AbstractGUICallback {
    ...
    public void init() {
        ...
        this.addElement(new OxygenGUIText(4, 5, ClientReference.localize("oxygen_groups.gui.callback.invitePlayer"), GUISettings.get().getTextScale(), GUISettings.get().getEnabledTextColor()));   
        this.addElement(new OxygenGUIText(6, 18, ClientReference.localize("oxygen_groups.gui.callback.invitePlayer.request"), GUISettings.get().getSubTextScale(), GUISettings.get().getEnabledTextColor()));  
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (mouseButton == 0) {
            if (element == this.cancelButton)
                ...
                GroupsManagerClient.instance().getGroupDataManager().inviteToGroupSynced(
                        OxygenHelperClient.getPlayerSharedData(this.usernameField.getTypedText()).getIndex());
                this.close();
            }
        }
    }
