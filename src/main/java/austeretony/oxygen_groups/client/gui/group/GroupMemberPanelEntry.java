...
public class GroupMemberPanelEntry extends OxygenWrapperPanelEntry<UUID> {

    private String lastActivity, dimension;
    ...
    public GroupMemberPanelEntry(PlayerSharedData sharedData) {
    ...
    public void draw(int mouseX, int mouseY) {
        if (this.isVisible()) {  
            ...
            if (this.isLeader) {
                ...
                drawCustomSizedTexturedRect(this.getWidth() - 20, 2, 0, 0, 6, 6, 6, 6);  
            }
    ...
    public void drawTooltip(int mouseX, int mouseY) {
        if (this.offline && mouseX >= this.getX() + 7 && mouseY >= this.getY() + 4 && mouseX < this.getX() + 10 && mouseY < this.getY() + 7)
            this.drawTooltip(mouseX, mouseY, this.lastActivity);
